...
try:
    ...
    print("Error: Make sure you are running main.py from the project root directory"
...
# Try to import RAPIDS cuML for NVIDIA GPU support
try:
    import cudf
    import cuml
    ...
    print("RAPIDS cuML found - NVIDIA GPU acceleration is available")
    USE_GPU = True
    GPU_TYPE = "NVIDIA"
except ImportError:
    print("RAPIDS cuML not found - trying alternative GPU acceleration options")
    
    # Try to import sklearnex for Intel GPU acceleration
    try:
        from sklearnex import patch_sklearn
        patch_sklearn()
        print("Intel scikit-learn-intelex found - Intel GPU/CPU acceleration is available")
        USE_GPU = True
        GPU_TYPE = "INTEL"
    except ImportError:
        print("No GPU acceleration libraries found - using standard CPU processing")
...
class ModelFactory:
    ...
    def create_model(model_type="random_forest", **kwargs):
        ...
        if USE_GPU:
            if GPU_TYPE == "NVIDIA":
                ...
                if model_type == "random_forest":
                    ...
                    print(f"Model type '{model_type}' not supported on GPU. Using Random Forest instead.")
        ...
        if model_type == "random_forest":
            ...
            print(f"Unknown model type '{model_type}'. Using Random Forest as default.")
...
class ExperimentRunner:
        ...
        if USE_GPU:
            ...
            if GPU_TYPE == "NVIDIA":
                ...
                try:
                    ...
                    print(f"Available GPU memory: {cupy.cuda.runtime.memGetInfo()[0]/1e9:.2f} GB")
    ...
    def _run_single_experiment(self, raw_df: pd.DataFrame, filter_type: str) -> tuple:
        ...
        print(f"Training data shape after preprocessing: {X_train_2d.shape}")
        ...
        if USE_GPU and GPU_TYPE == "NVIDIA":
            try:
                ...
                if X_train_2d.shape[0] * X_train_2d.shape[1] < 1e8:  # Size check to avoid OOM
                    ...
                    print("Data too large for GPU memory, using CPU")
                    USE_GPU = False
            except Exception as e:
        ...
        print(f"Using {model_name} model for {filter_type} filter")
...
def main():
    ...
    # Handle GPU disabling if requested
    global USE_GPU
    if args.no_gpu:
        print("GPU acceleration disabled by command line argument")
        USE_GPU = False
